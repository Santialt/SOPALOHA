...
test("requireInternalAccess does not trust spoofed private forwarded IPs over a public direct connection", async () => {
  ...
  for (const spoofedIp of privateSpoofs) {
    ...
    assert.equal(
      ...
      401,
      `spoofed ${spoofedIp} must be rejected`,
    );
    assert.match(
...
test("invalid TRUST_PROXY blocks startup instead of falling back to permissive Express defaults", async () => {
  ...
  const result = spawnSync(
    ...
    [
      "-e",
      `process.env.NODE_ENV="test"; process.env.AUTH_SESSION_SECRET="secret"; process.env.SQLITE_DB_PATH=${JSON.stringify(sqliteDbPath)}; process.env.TRUST_PROXY="true"; require("./apps/api/src/app");`,
    ],
    {
      cwd: repoRoot,
      encoding: "utf8",
