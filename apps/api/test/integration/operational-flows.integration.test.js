...
test('critical operational CRUD flows work against a temporary SQLite database', async (t) => {
  ...
  await t.test('locations support create, validation failure, update, list, and delete', async () => {
    const invalidCreate = await harness.authedRequest(adminUser, 'POST', '/locations', {
      body: {}
    });
    ...
    const createResult = await harness.authedRequest(adminUser, 'POST', '/locations', {
      body: {
        name: 'Local Palermo',
        city: 'Buenos Aires',
        usa_nbo: true,
    ...
    const listResult = await harness.authedRequest(adminUser, 'GET', '/locations');
    assert.equal(listResult.status, 200);
    assert.ok(listResult.body.some((location) => location.id === locationId));

    const updateResult = await harness.authedRequest(adminUser, 'PUT', `/locations/${locationId}`, {
      body: {
  ...
  await t.test('devices support create, validation failure, update, list, and delete', async () => {
    const invalidCreate = await harness.authedRequest(adminUser, 'POST', '/devices', {
      body: {
    ...
    assert.equal(invalidCreate.status, 400);
    assert.match(invalidCreate.body.message, /Field 'ram_gb' must be a number/);

    const createResult = await harness.authedRequest(adminUser, 'POST', '/devices', {
      body: {
        location_id: locationId,
    ...
    const listResult = await harness.authedRequest(adminUser, 'GET', `/locations/${locationId}/devices`);
    assert.equal(listResult.status, 200);
    assert.ok(listResult.body.some((device) => device.id === deviceId));

    const updateResult = await harness.authedRequest(adminUser, 'PUT', `/devices/${deviceId}`, {
      body: {
        location_id: locationId,
        name: 'POS Front',
        type: 'pos_terminal',
  ...
  await t.test('incidents support create, validation failure, update, list, and delete', async () => {
    const invalidCreate = await harness.authedRequest(adminUser, 'POST', '/incidents', {
      body: {
        location_id: locationId,
        device_id: deviceId,
        date: '2026-02-01',
    ...
    const createResult = await harness.authedRequest(adminUser, 'POST', '/incidents', {
      body: {
        location_id: locationId,
        device_id: deviceId,
    ...
    const listResult = await harness.authedRequest(adminUser, 'GET', `/locations/${locationId}/incidents`);
    assert.equal(listResult.status, 200);
    assert.ok(listResult.body.some((incident) => incident.id === incidentId));

    const updateResult = await harness.authedRequest(adminUser, 'PUT', `/incidents/${incidentId}`, {
      body: {
        location_id: locationId,
        device_id: deviceId,
  ...
  await t.test('tasks support create, assignment validation, update, list, and delete', async () => {
    const invalidCreate = await harness.authedRequest(adminUser, 'POST', '/tasks', {
      body: {
    ...
    assert.equal(invalidCreate.status, 400);
    assert.equal(invalidCreate.body.message, 'Assigned user is invalid or inactive');

    const createResult = await harness.authedRequest(adminUser, 'POST', '/tasks', {
      body: {
    ...
    const listResult = await harness.authedRequest(adminUser, 'GET', `/locations/${locationId}/tasks`);
    assert.equal(listResult.status, 200);
    assert.ok(listResult.body.some((task) => task.id === taskId));

    const updateResult = await harness.authedRequest(adminUser, 'PUT', `/tasks/${taskId}`, {
      body: {
  ...
  await t.test('on-call shifts support create, validation failure, current shift logic, update, and delete', async () => {
    const now = new Date();
    const activeStart = new Date(now.getTime() - 30 * 60 * 1000);
    const activeEnd = new Date(now.getTime() + 30 * 60 * 1000);

    const invalidCreate = await harness.authedRequest(adminUser, 'POST', '/on-call-shifts', {
      body: {
    ...
    assert.equal(invalidCreate.status, 400);
    assert.equal(invalidCreate.body.message, 'start_at must be earlier than end_at');

    const createResult = await harness.authedRequest(adminUser, 'POST', '/on-call-shifts', {
      body: {
    ...
    const currentResult = await harness.authedRequest(adminUser, 'GET', '/on-call-shifts/current');
    ...
    const updateResult = await harness.authedRequest(adminUser, 'PUT', `/on-call-shifts/${shiftId}`, {
      body: {
  ...
  await t.test('critical resources can be deleted in dependency order', async () => {
    const deleteTask = await harness.authedRequest(adminUser, 'DELETE', `/tasks/${taskId}`);
    assert.equal(deleteTask.status, 204);

    const deleteIncident = await harness.authedRequest(adminUser, 'DELETE', `/incidents/${incidentId}`);
    assert.equal(deleteIncident.status, 204);

    const deleteDevice = await harness.authedRequest(adminUser, 'DELETE', `/devices/${deviceId}`);
    assert.equal(deleteDevice.status, 204);

    const deleteShift = await harness.authedRequest(adminUser, 'DELETE', `/on-call-shifts/${shiftId}`);
    assert.equal(deleteShift.status, 204);

    const deleteLocation = await harness.authedRequest(adminUser, 'DELETE', `/locations/${locationId}`);
