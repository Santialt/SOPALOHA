...
test('TeamViewer backend hardening covers preview, import, degradation, and current route authorization', async (t) => {
  ...
  await t.test('unauthenticated TeamViewer routes are rejected and current authenticated tech access remains allowed', async () => {
    teamviewerFetch.reset();

    const unauthenticated = await harness.request('GET', '/teamviewer/import-preview');
    assert.equal(unauthenticated.status, 401);
    assert.equal(unauthenticated.body.message, 'Authentication required');

    const existingLocationId = harness.db
      .prepare(`INSERT INTO locations (name, status) VALUES ('Local Reused', 'active')`)
      .run().lastInsertRowid;
    ...
    teamviewerFetch.setHandler(async (url) => {
      ...
      if (parsedUrl.pathname === '/api/v1/devices') {
        return jsonResponse({
          devices: [
            { groupid: 'g-existing', alias: 'Server Principal', remotecontrol_id: '123 456 789' },
            { groupid: 'g-existing', alias: 'Caja 1', device_id: 'device-pos-1' },
            { groupid: 'g-new', device_id: 'device-no-alias' },
            { groupid: 'missing-group', alias: 'Sin Grupo', remotecontrol_id: '999' }
          ]
        });
      }
    ...
    const preview = await harness.authedRequest(techUser, 'GET', '/teamviewer/import-preview');
    ...
    const createdCandidate = preview.body.devices.find((row) => row.alias === 'Device device-no-alias');
    assert.deepEqual(
      {
    ...
    const importResult = await harness.authedRequest(techUser, 'POST', '/teamviewer/import');
    ...
    const importedDevice = harness.db
      .prepare(`SELECT * FROM devices WHERE location_id = ? AND name = 'Device device-no-alias'`)
  ...
  await t.test('TeamViewer imported-case flows handle pagination, duplicates, partial payloads, and current tech mutation access', async () => {
    teamviewerFetch.reset();
    ...
    const importCasesResult = await harness.authedRequest(techUser, 'POST', '/teamviewer/import-cases', {
      body: {
    ...
    const listImportedCases = await harness.authedRequest(techUser, 'GET', '/teamviewer/imported-cases');
    assert.equal(listImportedCases.status, 200);
    assert.equal(listImportedCases.body.length, 2);

    const resolvedNames = listImportedCases.body.map((row) => row.resolved_location_name).sort();
    assert.deepEqual(resolvedNames, ['Local Nuevo', 'Local Reused']);

    const manualCase = await harness.authedRequest(techUser, 'POST', '/teamviewer/imported-cases', {
      body: {
  ...
  await t.test('TeamViewer upstream failures degrade to stable 502 API responses instead of generic crashes', async () => {
    teamviewerFetch.reset();
    teamviewerFetch.setHandler(async () => jsonResponse({ error: 'service unavailable' }, { status: 503 }));

    const result = await harness.authedRequest(adminUser, 'GET', '/teamviewer/import-preview');

    assert.equal(result.status, 502);
    assert.equal(result.body.code, 'TEAMVIEWER_UPSTREAM_ERROR');
    assert.equal(result.body.message, 'TeamViewer service is temporarily unavailable');
  });

  await t.test('TeamViewer auth failures degrade to stable 502 API responses with TeamViewer-specific messaging', async () => {
    teamviewerFetch.reset();
    teamviewerFetch.setHandler(async () => jsonResponse({ error: 'unauthorized' }, { status: 401 }));

    const result = await harness.authedRequest(adminUser, 'POST', '/teamviewer/import-cases', {
      body: {
        from_date: '2026-03-01',
        to_date: '2026-03-31'
      }
