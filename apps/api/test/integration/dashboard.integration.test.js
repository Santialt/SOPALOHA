...
test('GET /dashboard/summary returns operational incident metrics', async () => {
  ...
  try {
    ...
    const locationA = harness.db
      .prepare(`INSERT INTO locations (name, status) VALUES ('Local Centro', 'active')`)
      .run().lastInsertRowid;
    const locationB = harness.db
      .prepare(`INSERT INTO locations (name, status) VALUES ('Local Norte', 'active')`)
      .run().lastInsertRowid;
    ...
    harness.db
      ...
      .run(locationA, formatDateOnly(withinWindow), 'Aloha no abre', 'Detalle', 'aloha', 'open', user.id, user.id);
    harness.db
      .prepare(
        `
          INSERT INTO incidents (
      ...
      .run(locationA, formatDateOnly(withinWindow), 'Error fiscal', 'Detalle', 'fiscal', 'closed', user.id, user.id);
    harness.db
      .prepare(
        `
          INSERT INTO incidents (
      ...
      .run(locationB, formatDateOnly(withinWindow), 'Aloha lento', 'Detalle', 'aloha', 'closed', user.id, user.id);
    harness.db
      .prepare(
        `
          INSERT INTO incidents (
      ...
      .run(locationB, formatDateOnly(outsideWindow), 'Incidente viejo', 'Detalle', 'printer', 'open', user.id, user.id);

    const loginResult = await harness.login(credentials.email, credentials.password);
    assert.equal(loginResult.status, 200);

    const result = await harness.requestWithSession('GET', '/dashboard/summary', loginResult.sessionCookie);
