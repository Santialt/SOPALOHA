...
const EXPLORER_CACHE_TTL_MS = 15000;
...
async function buildExplorerData() {
  const now = Date.now();
  if (explorerCache && explorerCache.expires_at > now) {
    return explorerCache.payload;
  }
  ...
  if (!explorerPromise) {
    explorerPromise = (async () => {
      ...
      explorerCache = {
        ...
        expires_at: Date.now() + EXPLORER_CACHE_TTL_MS
      };
